...
## SNR-SBX (limited-module) smoke scope

The SNR-SBX teller account exposes **only Customers, Accounts, Transactions and
Reports** — there are **no Products or Loans modules**, and the tenant has no
cash deposit/withdrawal, external-transfer, or loan/interest transaction data.
Running the full suite yields ~140 not-applicable failures, so SNR-SBX is run
through a dedicated scoped wrapper: **`run_snr_sbx_smoke.sh`**.
...
present in SNR-SBX; safe to run, then removed via `rebot`):
| Sub-tests | Reason |
|-----------|--------|
